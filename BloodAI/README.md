...
## Recent changes
- **New `ask_user` tool**: lets the AI pause mid-task and ask a genuine clarifying question. Posts a thread (or replies directly in DMs/existing threads) that @mentions the asker, with 2-5 clickable option buttons plus an "Other / custom answer" button (opens a modal, freeform text — the calling AI interprets things like "1 + 3, I'd rather do both" itself from the raw answer) and a "Cancel" button. Buttons only: only the person who triggered the question can press them, and typed replies in the thread are ignored (the old free-text discussion flow answered without context, so it was removed). Times out per the `ask_user_*` config keys if nobody answers.
